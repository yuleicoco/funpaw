//
//  ExchangePremissViewController.m
//  petegg
//
...
@interface ExchangePremissViewController ()
...
@implementation ExchangePremissViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self setNavTitle:@"修改访问规则"];
...
-(void)initChangeView{
    ...
    _ruleNameTextfield.text = _exchangeRuleName;
    ...
    _priceTextfield.text = _exchangePrice;
    ...
    _tsnumTextfield.text = _exchangeToushi;
   ...
   // [_allbodyBtn addSubview:_gouImage];
    ...
    if ([_exchangeObject isEqualToString:@"all"]) {
        [_gouImage removeFromSuperview];
        [_allbodyBtn addSubview:_gouImage];
        _allbodyBtn.selected = YES;
        _friendBtn.selected = NO;
        _zdBtn.selected = NO;
    }else if ([_exchangeObject isEqualToString:@"friend"]){
        [_gouImage removeFromSuperview];
    ...
    }else if ([_exchangeObject isEqualToString:@"zd"]){
        [_gouImage removeFromSuperview];
...
//    }];
    ...
    [[AFHttpClient sharedAFHttpClient]ruleModifyInfoWithrid:_exchangeRid rulesname:_ruleNameTextfield.text object:objectStr friends:friendstr price:_priceTextfield.text tsnum:_tsnumTextfield.text complete:^(BaseModel *model) {
        [self hideHud];
        [[AppUtil appTopViewController] showHint:model.retDesc];
        [self.navigationController popViewControllerAnimated:YES];
        [[NSNotificationCenter defaultCenter]postNotificationName:@"premissshuaxin" object:nil];
