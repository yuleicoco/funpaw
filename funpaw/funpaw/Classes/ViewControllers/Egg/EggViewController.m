...
- (void)viewDidLoad {
    ...
    // sephone
    [SephoneManager addProxyConfig:[AccountManager sharedAccountManager].loginModel.sipno password:[AccountManager sharedAccountManager].loginModel.sippw domain:@"sip.smartsuoo.com"];
...
-(void)UIchangeMothod
{
    ...
    if ([strState isEqualToString:@"ds000"]) {
        [bgImage setImage:[UIImage imageNamed:@"newnewtipssss.png"]];
        addBtn.hidden = NO;
        SbgImage.hidden = YES;
        [self showBarBtn:YES];
        return;
